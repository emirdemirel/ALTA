...
nj=30
gmm_model=tri3b_prons      # this is the source gmm-dir that we'll use for alignments; it

train_set=train_damp_dali_music
test_sets="dev test_damp test_dali jamendo"

nnet3_affix=_mstrenet
...
gmm_dir=exp${affix}/${gmm_model}
ali_dir=exp${affix}/tri3b_ali_${train_set}_sp
lat_dir=exp${affix}/chain_${chain_affix}/tri3b_${train_set}_lats
...
#Configuration
dir=exp$affix/chain${chain_affix}/mstrenet

train_data_dir=data/${train_set}_sp_hires
train_ivector_dir=exp${affix}/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
lores_train_data_dir=data/${train_set}_sp
lang=data/lang${affix}_chain
tree_dir=exp${affix}/chain${chain_affix}/tree
...
if [ $stage -le 3 ]; then
  ...
  local/nnet3/run_ivector_training.sh \
    --stage 0 --nj $nj --nnet3_affix $nnet3_affix --affix $affix\
    --train_set ${train_set}_sp_hires 
fi
...
if [ $stage -le 4 ]; then
  ...
  for datadir in ${train_set}_sp; do
    ...
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "${nspk}" \
      data/${datadir}_hires exp${affix}/nnet3${nnet3_affix}/extractor \
      exp${affix}/nnet3${nnet3_affix}/ivectors_${datadir}_hires
  done
fi
...
if [ $stage -le 6 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    data/${train_set}_sp data/lang${affix}_prons $gmm_dir $ali_dir
fi
...
if [ $stage -le 7 ]; then
  ...
  steps/align_fmllr_lats.sh --nj 40 --cmd "$train_cmd" \
    data/${train_set} data/lang${affix}_prons $gmm_dir $lat_dir
...
if [ $stage -le 8 ]; then
  ...
  # once, the second one has zero or more repeats.]
    cp -r data/lang_4G${affix}_prons $lang
...
for f in $train_data_dir/feats.scp exp${affix}/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires/ivector_online.scp \
...
if [ $stage -le 10 ]; then
  ...
  # Convolutional front-end for dimension reduction
  ...
  # Multistream TDNN blocks with distinct structures
...
graph_dir=$dir/graph_4G$affix
if [ $stage -le 12 ]; then
  utils/mkgraph.sh --self-loop-scale 1.0 --remove-oov data/data/lang_4G${affix}_prons $dir $graph_dir
...
if [[ $stage -le 13 ]]; then
  ...
  echo
  echo "============================="
  echo "---- Extract I-vectors for the test set ----"
  echo "=====  $(date +"%D_%T") ====="
  echo

  for datadir in ${testsets}; do
    nspk=$(wc -l <data/${datadir}_hires/spk2utt)
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "${nspk}" \
      data/${datadir}_hires exp${affix}/nnet3${nnet3_affix}/extractor \
      exp${affix}/nnet3${nnet3_affix}/ivectors_${datadir}_hires
    ...
    (
      ...
      nspk=$(wc -l <data/${data}_hires/spk2utt)
        steps/nnet3/decode.sh \
          ...
          --online-ivector-dir exp${affix}/nnet3${nnet3_affix}/ivectors_${data}_hires \
          $graph_dir data/${data}_hires ${dir}/decode_4G_${data_affix} || exit 1
  )  
  done
