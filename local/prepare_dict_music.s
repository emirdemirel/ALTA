...
cat $dir/lexicon2_raw.txt  \
   <( echo "MM M"
      echo "<UNK> SPN" 
      echo "<music> <MUSIC>" >> $dir/lexicon.txt
...
./steps/dict/apply_g2p_phonetisaurus.sh --nbest 3 \
    data/local/dict$affix/oov_words.txt conf/g2p data/local/dict$affix/
cut -d$'\t' -f1,3 data/local/dict$affix/lexicon.lex > data/local/dict$affix/lex
sed -e 's/\t/ /g' data/local/dict$affix/lex > data/local/dict$affix/oov_lexicon.txt
cat data/local/dict$affix/oov_lexicon.txt data/local/dict$affix/lexicon_raw.txt | sort -u > data/local/dict$affix/lexicon.txt
