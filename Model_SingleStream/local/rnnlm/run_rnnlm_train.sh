...
train=../s4_cnn/data/$mic/train30/text
#train=data/$mic/text_rnnlm
dev=data/$mic/dev/text
wordlist=data/lang/words.txt
text_dir=data/rnnlm_new/text
dir=exp/rnnlm_cnn_tdnn_$affix
mkdir -p $dir/config
set -e
...
if [ $stage -le 0 ]; then
  mkdir -p $text_dir
  cat $train | cut -d ' ' -f2- > $text_dir/train30.txt
...
if [ $stage -le 3 ]; then
  ...
  rnnlm/train_rnnlm.sh --embedding_l2 $embedding_l2 \
                       --stage $train_stage \
                       --num-epochs $epochs --cmd $cmd $backstitch_opt exp/rnnlm_cnn_tdnn_$affix
fi
